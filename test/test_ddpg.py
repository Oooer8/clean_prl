...
import numpy as np
import gym
from configs.ddpg_config import Config
...
def test_run(env_name="Pendulum-v1", num_episodes=1000, render_freq=50, plot_freq=10, save_freq=100):
    ...
    print(f"Starting training on {env_name} with DDPG")
    ...
    for episode in range(num_episodes):
        ...
        if episode % plot_freq == 0 and episode > 0:
            ...
            print(f"Reward plot saved to ddpg_training_rewards.png")
        
        # 定期保存模型
        if episode % save_freq == 0 and episode > 0:
            save_path = f"models/ddpg_{env_name.replace('-', '_')}_{episode}.pt"
            os.makedirs(os.path.dirname(save_path), exist_ok=True)
            agent.save(save_path)
            print(f"Model saved to {save_path}")
    
    # 训练结束，保存最终模型和图表
    final_save_path = f"models/ddpg_{env_name.replace('-', '_')}_final.pt"
    ...
    print("\n" + "="*50)
    print(f"Training completed for {env_name} with DDPG")
